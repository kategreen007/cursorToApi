...
function stringToHex(str) {
    ...
    const SEPARATOR = 1;
    const FIXED_SUFFIX_LENGTH = 0xBD;
    ...
    const hexString = (
        ...
        "2002A132F643A2F6964656150726F2F656475626F73733A1E0A1A636C617564652D332D352D736F6E6E65742D3230323431303232" +
        "22004A" +
...
app.post('/v1/chat/completions', async (req, res) => {
    try {
        ...
        const hexData = stringToHex(lastMessage.content);
        ...
        if (stream) {
            ...
            for await (const chunk of response.body) {
                ...
                if (chunk && Buffer.isBuffer(chunk) && 
                    ...
                    try {
                        // 跳过前7个字节的头部数据
                        const text = chunk.slice(7).toString('utf-8');
                        if (text.length > 0) {
                            // 发送内容块
            ...
            for await (const chunk of response.body) {
                ...
                if (chunk && Buffer.isBuffer(chunk) && 
                    ...
                    try {
                        // 跳过前7个字节的头部数据
                        text = text+chunk.slice(7).toString('utf-8');
